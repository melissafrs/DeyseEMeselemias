...
function FormFacade(data)
{
    ...
    {
        var curr = this;
        var items = this.data.scraped&&this.data.scraped.items?this.data.scraped.items:{};
        var oitems = this.data.facade&&this.data.facade.items?this.data.facade.items:{};
        var sitems = [];
    ...
    this.init = function(savedId)
    {
        ...
        var prefillId = urlparams.get('prefillId');
        if(prefillId)
        {
    ...
    this.load = function(divId)
    {
        ...
        this.init().then(function(){
            ...
            {
                if(curr.data.scraped) curr.render();
            ...
            if(fac.setting && fac.setting.currency)
            {
                ...
                var ver = curr.data.devEnv?new Date().getTime():6;
                curr.addLinkTag(host+'/css/neartail.css?nocache='+ver);
                if(curr.isEditMode())
                    curr.addLinkTag('/css/neartail.editor.css?nocache='+ver);
            }
        ...
        if(itmlen > 0)
        {
            ...
            this.__sections.forEach((sec, s)=>{
                sec.items.forEach(function(item, itmi){
                    ...
                    if(item.product && item.type=='TEXT' && oitem.choices)
                    {
                      item.type = 'LIST';
    ...
    params.categories = params.CATEGORIES = function()
    {
        ...
        secs.forEach(function(sec, s){
            ...
            if(nav && nav.navigation=='added' && prds.length>0)
            {
                ...
                var line = '<li class="mdc-image-list__item" onclick="'+ivk+'">';
